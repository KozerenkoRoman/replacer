...
const get Definitions= async (req, res, next) => {
    try {
        const  definitions= await #models#Service.get#Models#();
...
const get#Model#ById = async (req, res, next) => {
    try {
        ...
        const #model# = await #models#Service.get#Model#ById(id);
...
const patch#Model# = async (req, res, next) => {
    try {
        const id = req.params.id;
        if (!id) return res.status(400).send('Id is required');
        const #model# = await #models#Service.patch#Model#ById(id, req.body);
        if (!#model#) return res.status(404).send();
...
const delete#Model# = async (req, res, next) => {
    try {
        const id = req.params.id;
        if (!id) return res.status(400).send('Id is required');
        const exist = await #models#Service.delete#Model#ById(id);
